...
class CSNLanceDataset(Dataset):
    """
    Dataset for CSN (Chapman-Shaoxing-Ningbo) finetuning/evaluation.

    One 12-lead ECG per patient (no longitudinal stays, unlike MIMIC), so
    there's a single mode — every row in the split.

    Returns: {"waveform": FloatTensor(12, 5000), "label": FloatTensor(N,)}
    where N is the number of SNOMED-CT Dx classes (see dx_vocabulary_csn.json).

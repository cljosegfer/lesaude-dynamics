...
\begin{table}[t]
\caption{Contextualized Inference macro-AUROC on MIMIC-IV-ECG (Section~\ref{sec:contextualized}): predicting $\vy_t$ from $\mX_{t-1}, \mX_t, \vy_{t-1}$. Supervised has no mechanism to consume $\vy_{t-1}$ or $\mX_{t-1}$ and cannot be evaluated on this task. 95\% CIs are from bootstrap resampling; the Dynamics CI is pending.}
...
This is the static multi-label classification task, evaluated on a single ECG exam at a time with no longitudinal context. On MIMIC-IV-ECG we follow the Triage protocol established by \cite{strodthoff2024mimic}, which restricts evaluation to the first ECG of each patient's stay. We broaden this study beyond MIMIC by also finetuning on other diagnostic ECG datasets, e.g. CSN (Chapman--Shaoxing--Ningbo), %TODO: confirm CSN and list any further datasets to include here
each under its own native label space rather than the MIMIC-derived 76-cluster cardiac action space. 
...
\begin{table}[t]
\caption{Finetuned macro-AUROC classification performance across datasets (Section~\ref{sec:classification}), each finetuned on that dataset's own label space. CSN = Chapman--Shaoxing--Ningbo. 95\% CIs are from bootstrap resampling; entries are pending where not yet available.}
\label{tab:classification}
\centering
\begin{tabular}{@{}l cc@{}}
\toprule
 & \multicolumn{2}{c}{\textbf{Macro-AUROC (Finetuned)}} \\
\cmidrule(l){2-3}
\textbf{Method} & \textbf{MIMIC} & \textbf{CSN} \\
\midrule

Dynamics                  & 0.740 \small{[pending]} & \emph{pending} \\
Inverse Dynamics          & 0.7373 \small{[0.7252 -- 0.7510]} & 0.8958 \small{[0.8806 -- 0.9117]} \\
\addlinespace
Supervised & 0.7260 \small{[0.7140 -- 0.7386]} & 0.9215 \small{[0.9115 -- 0.9327]} \\
SSL baseline    & \emph{pending} & \emph{pending} \\
\bottomrule
\end{tabular}
\end{table}
...
Here we test generalization beyond MIMIC-IV-ECG itself. We match ICD-10 codes from MIMIC to the labels available in the smaller external datasets described in Section~\ref{sec:classification}, restricting evaluation to the codes common to both label sets and ignoring the rest. Because this code-matching can be more or less permissive, we report results at three matching thresholds, Low, Medium, and High, %TODO: define precisely what the Low/Medium/High matching threshold controls
which trade off how many labels survive the intersection against how exact the semantic correspondence between a MIMIC cluster and its external counterpart is. Onto this shifted label distribution we transfer, without any further finetuning, the MIMIC-finetuned Dynamics model, the MIMIC-finetuned Inverse Dynamics model, the MIMIC-finetuned naive SSL baseline, and the MIMIC-trained supervised classifier. This tests whether the structure imposed by the action-conditioned latent space transfers more robustly across acquisition sites and populations than a standard supervised classifier, complementing the low-data results of the original study with a distribution-shift, rather than sample-size, stress test. Table~\ref{tab:distshift} reports zero-shot macro-AUROC on CSN under all three thresholds.

\begin{table}[t]
\caption{Zero-shot Distribution Shift macro-AUROC on CSN (Chapman--Shaoxing--Ningbo), Section~\ref{sec:distshift}. All methods are pretrained/finetuned on MIMIC-IV-ECG only and transferred without further training, under three ICD-10 label-matching thresholds. 95\% CIs are from bootstrap resampling; entries are pending where not yet available.}
\label{tab:distshift}
\centering
\begin{tabular}{@{}l ccc@{}}
\toprule
 & \multicolumn{3}{c}{\textbf{Macro-AUROC (CSN)}} \\
\cmidrule(l){2-4}
\textbf{Method} & \textbf{Medium} & \textbf{Low} & \textbf{High} \\
\midrule
Dynamics                  & \emph{pending} & \emph{pending} & \emph{pending} \\
Inverse Dynamics          & 0.8591 \small{[0.8346 -- 0.8794]} & 0.8203 \small{[0.7989 -- 0.8380]} & 0.8498 \small{[0.8333 -- 0.8650]} \\
\addlinespace
Supervised                & 0.8220 \small{[0.7894 -- 0.8493]} & 0.7822 \small{[0.7537 -- 0.8060]} & 0.8194 \small{[0.8016 -- 0.8367]} \\
SSL baseline              & \emph{pending} & \emph{pending} & \emph{pending} \\
\bottomrule
\end{tabular}
\end{table}

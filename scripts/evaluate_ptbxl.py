"""
Test-set Bootstrap Evaluation on PTB-XL
======================================
Loads a finetuned/supervised checkpoint and reports macro AUROC with 95%
bootstrap CI on the PTB-XL held-out test fold (strat_fold == 10). PTB-XL is
one 12-lead ECG per record (no CPSC-style sliding-window segmentation), so
unlike evaluate_cpsc.py this needs no window-to-record aggregation step.

Example
-------
HYDRA_FULL_ERROR=1 python scripts/evaluate_ptbxl.py \
    ++evaluate.ckpt_path=checkpoints/finetune_ptbxl_inv.ckpt
"""
...
def main(cfg):
    ...
    ds = PTBXLLanceDataset(cfg.lance_path, split="test")
    loader = DataLoader(
        ds,
    ...
    print(f"Test set: {len(ds)} samples  ({num_classes} SCP classes)")

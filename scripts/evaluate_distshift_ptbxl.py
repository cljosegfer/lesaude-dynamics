"""
Distribution Shift Evaluation: MIMIC -> PTB-XL
==============================================
...
PTB-XL waveforms. Scoring is restricted to the ICD-10 clusters that have a
genuine PTB-XL counterpart (src/dataset/icd10_crosswalk.py +
src/dataset/scp_to_icd10_ptbxl.csv), using PTB-XL's own SCP-ECG labels,
OR-reduced onto those clusters, as ground truth.

This is the transfer/generalization stress test of papel/experiments.tex's
"Distribution Shift" study, run a second time on a different acquisition
site/label system (the CSN counterpart is scripts/evaluate_distshift_csn.py).
PTB-XL's crosswalk reaches an ICD-10 cluster CSN's couldn't (I25, chronic
ischemic heart disease, at min_confidence=low) thanks to PTB-XL's much
stronger MI/ischemia label coverage.

Example
-------
# Inverse-dynamics MIMIC-finetuned classifier (the config default)
HYDRA_FULL_ERROR=1 python scripts/evaluate_distshift_ptbxl.py

# Compare a different MIMIC-trained checkpoint
HYDRA_FULL_ERROR=1 python scripts/evaluate_distshift_ptbxl.py \\
    ++evaluate.ckpt_path=archive/supervised_0.ckpt

# Include the low-confidence crosswalk rows too (adds I25, I51)
HYDRA_FULL_ERROR=1 python scripts/evaluate_distshift_ptbxl.py ++min_confidence=low
"""
...
from dataset.ptbxl_dataset import PTBXLLanceDataset
from dataset.icd10_crosswalk import build_intersection, project_csn_labels
from models.resnet1d import ResNet1d

_CROSSWALK_PATH = Path(__file__).parent.parent / "src" / "dataset" / "scp_to_icd10_ptbxl.csv"
...
def _run_inference(backbone, projector, loader, device):
    """Returns MIMIC-space 76-dim sigmoid predictions and raw 71-dim PTB-XL labels."""
    all_preds, all_ptbxl_labels = [], []
    for batch in tqdm(loader, desc="Inference"):
        x = batch["waveform"].to(device)
        emb = backbone(x)
        logits = projector(emb)
        all_preds.append(F.sigmoid(logits).cpu().float().numpy())
        all_ptbxl_labels.append(batch["label"].cpu().numpy())
    return np.concatenate(all_preds), np.concatenate(all_ptbxl_labels)
...
@hydra.main(version_base="1.3", config_path="../configs", config_name="distshift_ptbxl")
def main(cfg):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device: {device}")

    mimic_vocab = json.load(open(cfg.mimic_vocabulary_path))
    ptbxl_vocab = json.load(open(cfg.ptbxl_vocabulary_path))
    xwalk = build_intersection(
        mimic_vocab, ptbxl_vocab,
        min_confidence=cfg.min_confidence,
        crosswalk_path=_CROSSWALK_PATH,
        code_column="scp_code",
    )
    ...
    ds = PTBXLLanceDataset(cfg.ptbxl_lance_path, split="test")
    loader = DataLoader(
        ds,
    ...
    print(f"PTB-XL test set: {len(ds)} samples")

    preds_76, ptbxl_labels = _run_inference(backbone, projector, loader, device)

    preds_k = preds_76[:, xwalk["mimic_indices"]]
    targets_k = project_csn_labels(ptbxl_labels, xwalk["csn_groups"])

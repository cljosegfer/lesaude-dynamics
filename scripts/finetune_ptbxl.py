"""
Fine-tuning / Linear Probing on PTB-XL
================================================================
Loads the encoder checkpoint pretrained on MIMIC-IV-ECG (default:
archive/pretrain_val.ckpt, the inverse-dynamics pretrained model) and
fine-tunes it for multi-label SCP-ECG code classification on PTB-XL — the
transfer counterpart to scripts/finetune.py's MIMIC-IV-ECG protocol, and the
same protocol used for CSN/CPSC (scripts/finetune_csn.py/finetune_cpsc.py).
...
-------
# Full fine-tune
HYDRA_FULL_ERROR=1 python scripts/finetune_ptbxl.py

# Linear probe
HYDRA_FULL_ERROR=1 python scripts/finetune_ptbxl.py ++freeze_encoder=true

Resuming an interrupted run
----------------------------
python scripts/finetune_ptbxl.py \\
...
@hydra.main(version_base="1.3", config_path="../configs", config_name="finetune_ptbxl")
def main(cfg):
    num_classes = len(json.load(open(cfg.vocabulary_path)))
    print(f"PTB-XL vocabulary: {num_classes} SCP classes")

    train_ds = PTBXLLanceDataset(cfg.lance_path, split="train", train_frac=cfg.train_frac, cache=cfg.cache)
    val_ds   = PTBXLLanceDataset(cfg.lance_path, split="val",   cache=cfg.cache)

"""
Fine-tuning / Linear Probing on CSN (Chapman-Shaoxing-Ningbo)
================================================================
Loads the encoder checkpoint pretrained on MIMIC-IV-ECG (default:
archive/inverse.ckpt, the inverse-dynamics pretrained model) and fine-tunes
it for multi-label SNOMED-CT diagnosis classification on CSN — the transfer
...
-------
# Full fine-tune
HYDRA_FULL_ERROR=1 python scripts/finetune_csn.py

# Linear probe
HYDRA_FULL_ERROR=1 python scripts/finetune_csn.py ++freeze_encoder=true

Resuming an interrupted run
----------------------------
python scripts/finetune_csn.py \\
...
def main(cfg):
    ...
    print(f"CSN vocabulary: {num_classes} Dx classes")

    train_ds = CSNLanceDataset(cfg.lance_path, split="train", train_frac=cfg.train_frac, cache=cfg.cache)
    val_ds   = CSNLanceDataset(cfg.lance_path, split="val",   cache=cfg.cache)
